...
class BiLSTM_CRF(nn.Module):
    ...
    def viterbi_decode(self, feats):
        ...
        for feat in feats:
            smat = (alpha.T + feat.unsqueeze(0).to(self.device) + self.transitions.to(self.device))  # (tagset_size, tagset_size)
            backtrace.append(smat.argmax(0))  # column_max
            alpha = log_sum_exp(smat)
        # backtrack
        smat = alpha.T + 0 + self.transitions[:, [self.tag_to_ix[STOP_TAG]]].to(self.device)
