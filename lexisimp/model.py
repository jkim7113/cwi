...
class BiLSTM_CRF(nn.Module):
    ...
    def compute_marginal_probabilities(self, feats): # not working as expected
        seq_len, tagset_size = feats.shape
    
        # Forward pass (alpha)
        alpha = torch.full((1, tagset_size), -10000.0, device=self.device)
        alpha[0][self.tag_to_ix[START_TAG]] = 0.0
        forward_vars = []
        ...
        alpha = log_sum_exp(alpha.T + self.transitions[:, [self.tag_to_ix[STOP_TAG]]])
        log_Z = alpha.flatten()[0]  # Log partition function
    
        # Backward pass (beta)
        beta = torch.full((1, tagset_size), -10000.0, device=self.device)
        beta[0][self.tag_to_ix[STOP_TAG]] = 0.0
        backward_vars = [beta] * seq_len  # initialize

        for i in reversed(range(seq_len)):
            feat = feats[i]
            beta = log_sum_exp(self.transitions + feat.unsqueeze(1) + beta)
            backward_vars[i] = beta
    
        # Marginal probabilities
        marginals = []
        for t in range(seq_len):
            log_marginal = forward_vars[t] + backward_vars[t] - log_Z
            marginals.append(torch.exp(log_marginal))
    
        # Shape: (seq_len, tagset_size)
        return torch.stack(marginals)
    ...
    def forward(self, sentence, chars, caps, chars2_length, d):
        ...
        if self.use_crf:
            ...
            probs = self.compute_marginal_probabilities(feats)
        else:
